...
export default function BillingClient({ orders, hotelName, adminUpiId }: { orders: any[], hotelName: string, adminUpiId: string }) {
  ...
  // Group UNPAID orders by mobile number and table id
  // Note: Only orders that have a mobile number and are UNPAID
  ...
  const filteredGroups = Object.keys(groups).filter(phone => phone.includes(searchPhone));
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div className="md:col-span-1 border-r border-slate-200 dark:border-slate-800 pr-6 min-h-[500px]">
        ...
        <div className="space-y-3">
          ...
          {filteredGroups.map(phone => {
            const customerOrders = groups[phone];
            ...
            return (
              <button 
                ...
                className={`w-full text-left p-4 rounded-xl border transition-all ${selectedMobile === phone ? 'bg-primary/5 border-primary shadow-sm' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-primary/50'}`}
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="font-bold flex items-center gap-2"><Phone className="w-3.5 h-3.5 text-slate-400" /> +91 {phone}</div>
          ...
          <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="flex justify-between items-center mb-6 pb-6 border-b border-slate-100 dark:border-slate-700">
              <div>
                ...
                <p className="text-slate-500 font-medium flex items-center gap-1"><Phone className="w-4 h-4"/> +91 {selectedMobile}</p>
              </div>
            </div>

            <div className="space-y-4 mb-8">
              {groups[selectedMobile].map(order => (
            ...
            {/* Total and QR Code */}
            {(() => {
              const netTotal = groups[selectedMobile].reduce((sum, o) => sum + o.totalAmount, 0);
              ...
              return (
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-800/50 text-center flex flex-col items-center">
                  ...
                  <button 
                    onClick={() => handleMarkPaid(groups[selectedMobile].map(o => o.id))}
